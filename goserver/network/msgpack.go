...
func (_m *Msgpack) SetConn(conn *net.Conn) {
	_m._conn = conn
}

func (_m *Msgpack) GetConn() net.Conn {
	return *_m._conn
}
...
func (_m *Msgpack) ReadString() []byte {
	_s := _m.ReadInt32()
	if _s+int32(_m._index) > int32(_m._max) {
		util.Log_error("read over max")
		return make([]byte, 0)
	}
	_m._index += int(_s)
